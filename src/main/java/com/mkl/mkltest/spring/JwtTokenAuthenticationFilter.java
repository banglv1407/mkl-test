...
public class JwtTokenAuthenticationFilter extends OncePerRequestFilter {
	...
	private String getToken(HttpServletRequest request) {
		...
		if (request.getParameter(SecurityTokenConfig.TOKEN_NAME) != null) {
			token = request.getParameter(SecurityTokenConfig.TOKEN_NAME);
		}

		if (request.getHeader(SecurityTokenConfig.TOKEN_NAME) != null) {
			token = request.getHeader(SecurityTokenConfig.TOKEN_NAME);
		}
		...
		if (WebUtils.getCookie(request, SecurityTokenConfig.TOKEN_NAME) != null) {
			token = WebUtils.getCookie(request, SecurityTokenConfig.TOKEN_NAME).getValue();
		}
